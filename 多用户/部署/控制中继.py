"""Forward the isolated gateway to the macOS loopback supervisor without holding secrets."""
...
async def _forward(client_reader: asyncio.StreamReader, client_writer: asyncio.StreamWriter) -> None:
    ...
    target_port = int(os.environ.get("XJ_RELAY_TARGET_PORT", "37654"))
...
async def main() -> None:
    server = await asyncio.start_server(_forward, "0.0.0.0", 8765, limit=64 * 1024)
    async with server:
